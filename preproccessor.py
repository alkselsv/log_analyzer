...
class Preprocessor:
    ...
    def __init__(self):
        ...
        self.columns = ['type', 'host', 'auto', 'cTm', 'date', 'remote_addr', 'session',
                        'page', 'http_referer', 'http_user_agent', 'access.status',
                        'access.request_time', 'access.bytes_sent', 'access.request',
                        'metrik.action', 'metrik.param', 'metrik.cookieEnabled',
                        'metrik.language', 'metrik.isMobile', 'metrik.platform']
    ...
    def __make_group__(self, dataframe):
        ...
        return dataframe.groupby(['session', 'time_round']).aggregate(self.aggs)\
            .reset_index()
    ...
    def __preproccess_data__(self, dataframe):
        ...
        dataframe['time_round'] = dataframe['datetime'].apply(lambda x: datetime(
    ...
    def __postproccess_data__(self, dataframe):
        ...
        dataframe['was_404'] = dataframe['access.status'].apply(
            lambda x: 1 if 404 in x else 0)  # флаг 404 кода в результатах
        dataframe['was_200'] = dataframe['access.status'].apply(
            lambda x: 1 if 200 in x else 0)  # флаг 200 кода в результатах

        dataframe['http_referer'] = dataframe['http_referer'].apply(
            self.__remove_nan__)  # удаляем nan из http_referer
        dataframe['http_referer'] = dataframe['http_referer'].apply(
            lambda x: x[x != '-'])  # удаляем '-' из http_referer
        dataframe['http_referer_is_empty'] = dataframe['http_referer'].apply(
            lambda x: int(len(x) == 0))  # флаг отсутствия рефереров

        dataframe['metrik.action'] = dataframe['metrik.action'].apply(
            self.__remove_nan__)  # удаляем NaN из metrik.action
        dataframe['was_action_ready'] = dataframe['metrik.action'].apply(lambda x: int(
            'ready' in x))  # флаг наличия действия ready в metrik.action
        dataframe['was_action_scroll'] = dataframe['metrik.action'].apply(lambda x: int(
            'scroll' in x))  # флаг наличия действия scroll в metrik.action
